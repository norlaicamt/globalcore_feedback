...
const DotsMenu = ({ onDelete }) => {
  ...
  return (
    <div ref={ref} style={{ position: "relative", display: "inline-block" }}>
      ...
      {open && (
        <div style={{ position: "absolute", right: 0, top: "34px", background: "white", border: "1px solid #E2E8F0", borderRadius: "10px", boxShadow: "0 8px 24px rgba(0,0,0,0.1)", zIndex: 100, minWidth: "130px", padding: "4px" }}>
          <button
            onClick={() => { onDelete(); setOpen(false); }}
            style={{ display: "block", width: "100%", padding: "8px 12px", background: "none", border: "none", borderRadius: "7px", textAlign: "left", fontSize: "12px", fontWeight: "600", color: "#B91C1C", cursor: "pointer", fontFamily: "inherit" }}
          >
            Delete Post
          </button>
        </div>
...
const HeaderFilter = ({ label, value, onChange }) => {
  ...
  return (
    <th style={{ ...thStyle, position: "relative" }}>
      ...
      >
        <span>{label}</span>
        <svg width="10" height="10" viewBox="0 0 24 24" fill={value ? "#1f2a56" : "currentColor"} style={{ transition: "transform 0.2s", transform: isOpen ? "rotate(180deg)" : "rotate(0deg)" }}>
          <path d="M7 10l5 5 5-5H7z" />
        </svg>
      </div>

      {isOpen && (
        <div ref={ref} style={{ position: "absolute", top: "100%", left: "10px", zIndex: 100, background: "white", padding: "10px", borderRadius: "8px", boxShadow: "0 10px 25px rgba(0,0,0,0.1)", border: "1px solid #E2E8F0", minWidth: "180px", marginTop: "4px" }}>
          <p style={{ margin: "0 0 8px 0", fontSize: "10px", fontWeight: "700", color: "#64748B", textTransform: "uppercase" }}>Filter {label}</p>
          <input
            autoFocus
            value={value}
            ...
            style={{ ...miniFilter, background: "white", border: "1.5px solid #E2E8F0", padding: "8px" }}
          />
          {value && (
            <button 
...
const AdminFeedbacks = () => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      {/* Search Header */}
      <div style={{ display: "flex", gap: "10px", alignItems: "center", background: "white", padding: "14px 16px", borderRadius: "12px", border: "1px solid #E2E8F0" }}>
        <input
      ...
      <div style={{ background: "white", borderRadius: "12px", border: "1px solid #E2E8F0", overflow: "visible" }}>
        {loading ? (
          <div style={{ textAlign: "center", padding: "40px", color: "#94A3B8", fontSize: "13px" }}>Loading feedback...</div>
        ) : (
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px" }}>
            <thead>
              <tr style={{ background: "#F8FAFC", borderBottom: "1px solid #E2E8F0" }}>
                <th style={thStyle}>#</th>
                <HeaderFilter label="Category Type" value={filters.category} onChange={v => setFilters({...filters, category: v})} />
                <HeaderFilter label="Establishment/Service" value={filters.establishment} onChange={v => setFilters({...filters, establishment: v})} />
                <HeaderFilter label="Author" value={filters.author} onChange={v => setFilters({...filters, author: v})} />
              ...
              {filtered.map((f, i) => (
                <tr key={f.id} style={{ borderBottom: "1px solid #F1F5F9" }}
                  ...
                  <td style={{ ...tdStyle, maxWidth: "220px" }}>
                    <p style={{ margin: 0, fontWeight: "700", color: "#1E293B", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  ...
                  <td style={{ ...tdStyle, color: f.is_anonymous ? "#94A3B8" : "#1E293B", fontWeight: f.is_anonymous ? "400" : "600" }}>
                    {f.user_name || "—"}
                    {f.is_anonymous && <span style={{ fontSize: "10px", marginLeft: "4px", fontStyle: "italic" }}>(Anon)</span>}
                  </td>
                  <td style={tdStyle}>{f.rating ? `${f.rating} / 5` : "—"}</td>
                  <td style={tdStyle}>{f.comments_count}</td>
                  <td style={{ ...tdStyle, color: "#94A3B8", whiteSpace: "nowrap" }}>{f.created_at?.split("T")[0]}</td>
                  <td style={{ ...tdStyle, textAlign: "right" }}>
                    <DotsMenu onDelete={() => handleDelete(f)} />
                  </td>
                </tr>
              ))}
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={8} style={{ padding: "32px", textAlign: "center", color: "#94A3B8", fontSize: "13px" }}>No feedback found.</td>
                </tr>
              )}
            </tbody>
          </table>
