...
const AdminFeedbacks = ({ theme, darkMode, adminUser }) => {
  ...
  const filtered = feedbacks.filter(f => {
    ...
    return tabMatch && searchMatch && programMatch && statusMatch;
  });
                ...
                {!isScopedAdmin && visibleColumns.program && (
                  <td style={{ padding: "16px 20px" }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <div style={{ fontSize: "10px", fontWeight: "700", color: "#64748B", textTransform: "uppercase", letterSpacing: "0.05em", opacity: 0.8 }}>{f.entity_name || "General"}</div>
                    </div>
                    <div style={{ fontSize: "14px", color: theme.text, marginTop: "2px", fontWeight: "600", letterSpacing: "-0.01em", lineHeight: "1.3" }}>
