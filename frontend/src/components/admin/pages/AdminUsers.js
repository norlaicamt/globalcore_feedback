...
const DotsMenu = ({ user, onToggle, onDelete }) => {
  ...
  return (
    <div ref={ref} style={{ position: "relative", display: "inline-block" }}>
      <button
        onClick={() => setOpen(o => !o)}
        style={{ width: "30px", height: "30px", display: "flex", alignItems: "center", justifyContent: "center", background: open ? "#F1F5F9" : "transparent", border: "1px solid transparent", borderRadius: "6px", cursor: "pointer", color: "#64748B", fontFamily: "inherit", transition: "all 0.15s" }}
      >
      ...
      {open && (
        <div style={{ position: "absolute", right: 0, top: "34px", background: "white", border: "1px solid #E2E8F0", borderRadius: "10px", boxShadow: "0 8px 24px rgba(0,0,0,0.1)", zIndex: 100, minWidth: "150px", padding: "4px" }}>
          <button
                ...
                <tr key={u.id} style={{ borderBottom: "1px solid #F1F5F9" }}
                  ...
                  <td style={tdStyle}>
                    <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                      {u.avatar_url
                        ? <img src={u.avatar_url} alt={u.name} style={{ width: "30px", height: "30px", borderRadius: "50%", objectFit: "cover", border: "1px solid #E2E8F0" }} />
                  ...
                  <td style={{ ...tdStyle, color: "#64748B" }}>{u.department || "—"}</td>
                  <td style={{ ...tdStyle, fontWeight: "600", color: "#374151" }}>{u.total_posts}</td>
                  ...
                  <td style={{ ...tdStyle, textAlign: "right" }}>
                    <DotsMenu user={u} onToggle={handleToggle} onDelete={handleDelete} />
                  </td>
                </tr>
              ))}
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={8} style={{ padding: "32px", textAlign: "center", color: "#94A3B8", fontSize: "13px" }}>No users found.</td>
                </tr>
              )}
            </tbody>
          </table>
